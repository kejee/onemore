...
@interface KKSegmentControlScrollView : UIScrollView
@end

@implementation KKSegmentControlScrollView
...
@interface KKSegmentControlLine : UIImageView
...
@interface KKSegmentControlButton : UIButton

- (instancetype)initWithImagePosition:(KKItemImagePosition)imagePosition;

@property (nonatomic) KKItemImagePosition imagePosition; // 图片位置
@property (nonatomic, assign) CGFloat imageTitleSpace; // 图片和文字之间的间距

@end

@implementation KKSegmentControlButton
...
- (void)initialize {
    _imagePosition = KKItemImagePositionLeft;
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case KKItemImagePositionLeft:
        case KKItemImagePositionDefault: {
...
- (void)setImagePosition:(KKItemImagePosition)imagePosition {
...
@interface KKSegmentControl()
@property (nonatomic, assign) KKSegmentControlTrackerStyle trackerStyle;
...
@property (nonatomic, weak) KKSegmentControlScrollView *itemScrollView;
@property (nonatomic, weak) KKSegmentControlButton *functionButton;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) KKSegmentControlButton *selectedButton;
...
@implementation KKSegmentControl
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        for (KKSegmentControlButton *button in self.buttons) {
    ...
    for (int i = 0; i < items.count; i++) {
        id object = items[i];
        NSAssert([object isKindOfClass:[NSString class]] || [object isKindOfClass:[UIImage class]] || [object isKindOfClass:[KKSegmentControlButtonItem class]], @"items中的元素类型只能是NSString、UIImage或KKSegmentControlButtonItem");
    ...
    if (self.buttons.count) {
        ...
        KKSegmentControlButton *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];

        // KKSegmentControlTrackerStyleTextZoom和KKSegmentControlTrackerStyleNothing样式跟tracker没有关联
...
- (void)insertItem:(KKSegmentControlButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%ld”",self.items.count);
    NSMutableArray *objects = self.items.mutableCopy;
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    ...
    if (itemIndex < self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        KKSegmentControlButton *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (void)setItem:(KKSegmentControlButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    if (item == nil) return;
    if (itemIndex < self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setItem:(KKSegmentControlButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex {
    [self setItem:item forItemIndex:itemIndex];
}

- (KKSegmentControlButtonItem *)itemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.items.count) {
        id object = [self.items objectAtIndex:itemIndex];
        NSAssert([object isKindOfClass:[KKSegmentControlButtonItem class]],@"itemIndex对应的item不是KKSegmentControlButtonItem类型，请仔细核对");
        return object;
    }
    return nil;
}
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        } else if ([content isKindOfClass:[KKSegmentControlButtonItem class]]) {
            KKSegmentControlButtonItem *item = (KKSegmentControlButtonItem *)content;
            [button setTitle:item.title forState:UIControlStateNormal];
            [button setImage:item.image forState:UIControlStateNormal];
            button.imagePosition = item.imagePosition;
            button.imageTitleSpace = item.imageTitleSpace;
...
- (void)setFunctionButtonWithItem:(KKSegmentControlButtonItem *)item forState:(UIControlState)state {
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(KKItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(KKItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(KKItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(KKItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forState:(UIControlState)state {
    [self.functionButton setTitle:title forState:state];
    [self.functionButton setImage:image forState:state];
    self.functionButton.imagePosition = imagePosition;
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    KKSegmentControlButton *button = [KKSegmentControlButton buttonWithType:UIButtonTypeCustom];
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        KKSegmentControlButtonItem *item = (KKSegmentControlButtonItem *)object;
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.image forState:UIControlStateNormal];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
    ...
    if (self.insert && animated) { // 是插入的新按钮,且需要动画
        // 取出上一个按钮
        KKSegmentControlButton *lastButton;
        if (index > 0) {
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != KKSegmentControlTrackerStyleTextZoom && self.trackerStyle != KKSegmentControlTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)setupSubViews {
    // 必须先添加分割线
    KKSegmentControlLine *dividingLine = [[KKSegmentControlLine alloc] init];
    ...
    KKSegmentControlScrollView *itemScrollView = [[KKSegmentControlScrollView alloc] init];
    ...
    KKSegmentControlButton *functionButton = [KKSegmentControlButton buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(KKSegmentControlButton *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == KKSegmentControlTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (KKSegmentControlButton *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)moveItemScrollViewWithSelectedButton:(KKSegmentControlButton *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(KKSegmentControlButton *)selectedButton {
...
- (void)delegatePerformMethodWithFromIndex:(NSUInteger)fromIndex toIndex:(NSUInteger)toIndex {
    if (self.delegate && [self.delegate respondsToSelector:@selector(segmentControl:itemSelectedFromIndex:toIndex:)]) {
        [self.delegate segmentControl:self itemSelectedFromIndex:fromIndex toIndex:toIndex];
    } else if (self.delegate && [self.delegate respondsToSelector:@selector(segmentControl:itemSelectedAtIndex:)]) {
        [self.delegate segmentControl:self itemSelectedAtIndex:toIndex];
    }
}

// 功能按钮的点击方法
- (void)functionButtonClicked:(KKSegmentControlButton *)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(segmentControl:functionButtonClicked:)]) {
        [self.delegate segmentControl:self functionButtonClicked:sender];
    }
}
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    switch (self.trackerFollowingMode) {
        case KKSegmentControlTrackerFollowingModeAlways:
            // 这个方法才开始移动跟踪器
            ...
            break;
        case KKSegmentControlTrackerFollowingModeHalf:{
            KKSegmentControlButton *fromButton;
            KKSegmentControlButton *toButton;
            if (progress > 0.5) {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == KKSegmentControlTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == KKSegmentControlTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == KKSegmentControlTrackerStyleTextZoom || self.trackerStyle == KKSegmentControlTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == KKSegmentControlTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(KKSegmentControlTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case KKSegmentControlTrackerStyleLine:
        case KKSegmentControlTrackerStyleLineLongerThanItem:
        case KKSegmentControlTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case KKSegmentControlTrackerStyleRoundedRect:
        case KKSegmentControlTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case KKSegmentControlTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<KKSegmentControlDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        KKSegmentControlButton *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(KKSegmentControlPermutationWay)permutationWay {
    _permutationWay = permutationWay;
    if (!_forceUseSettingSpacing) {
        if (_permutationWay == KKSegmentControlPermutationWayNotScrollEqualWidths) {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = KKSegmentControlTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = KKSegmentControlTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        } else if (button.currentTitle.length && button.currentImage && (button.imagePosition == KKItemImagePositionRight || button.imagePosition == KKItemImagePositionLeft || button.imagePosition == KKItemImagePositionDefault)) {
            contentW = textW + imageW + button.imageTitleSpace+button.contentEdgeInsets.left+button.contentEdgeInsets.right;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == KKItemImagePositionTop || button.imagePosition == KKItemImagePositionBottom)) {
    ...
    if (self.permutationWay == KKSegmentControlPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(KKSegmentControlButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == KKSegmentControlPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == KKSegmentControlPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
- (void)resetupTrackerFrameWithSelectedButton:(KKSegmentControlButton *)selectedButton {
    CGFloat trackerX;
    CGFloat trackerY;
    CGFloat trackerW;
    CGFloat trackerH;
...
@implementation KKSegmentControlButtonItem

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    KKSegmentControlButtonItem *item = [[KKSegmentControlButtonItem alloc] initWithTitle:title image:image imagePosition:KKItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(KKItemImagePosition)imagePosition {
    KKSegmentControlButtonItem *item = [[KKSegmentControlButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(KKItemImagePosition)imagePosition {
    if (self = [super init]) {
        self.title = title;
        self.image = image;
        self.imagePosition = imagePosition;
